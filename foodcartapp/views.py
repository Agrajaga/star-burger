...
def validate_field(data: dict, name: str, field_type: type) -> str | None:
    try:
        if data[name] is None:
            return f'{name}: Поле не может быть пустым'
        if not isinstance(data[name], field_type):
            return f'{name}: Ожидался тип поля {field_type}'
        if not data[name]:
            return f'{name}: Поле не может быть пустым'
    except KeyError:
        return f'{name}: Обязательное поле'
...
def register_order(request):
    ...
    if not order_data:
        return Response(['Пустой запрос'], status=400)
    errors = []
    ...
    if error:
        ...
        for err_id in err_ids:
            errors.append(f'products: Недопустимый первичный ключ {err_id}')
    if errors:
