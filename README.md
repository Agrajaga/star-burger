...
Сайт доступен по адресу https://burger.ramzes.pro
...
Скачайте код:
```sh
git clone https://github.com/Agrajaga/star-burger.git
```
...
- `DATABASE_URL` — строка подключения к базе данных, если этот параметр не указывать, то будет использоваться база SQLite. Формат строки можно посмотреть по адресу https://github.com/jazzband/dj-database-url#url-schema

### Быстрый деплой
Для автоматизации развертывания изменений на сервере можно использовать скрипт 
```bash
./deploy_starburger.sh
```
Скрипт выполнит следующие операции:
- получит изменения из Git репозитория
- установит библиотеки Python
- установит библиотеки Node.js
- пересоберет JS-код
- соберет статические файлы
- отмигрирует базу данных
